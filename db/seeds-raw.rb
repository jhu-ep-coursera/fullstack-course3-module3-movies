...
#add an example of an embedded class within the Actors 
class GMaps
  include HTTParty
...
Actor.collection.find(:placeOfBirth=>{:$exists=>true}).each do |doc|
  ...
  places=GMaps.get("",query:{address: placeOfBirth})["results"]
  ...
  if places[0]
    ...
    pob[:_id]=id[:_id]
  else #skipping GMaps and simulating
    pob={"_id"=>"Pasadena, CA, USA",
